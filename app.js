...
app.get('/', (req, res) => {
    ...
    res.send('<h1>Hola compañeros, esta en la página principal.</h1><a href="/marketing">marketing</a><a href="/developers">developers</a><a href="/contact">contact</a>')
})
...
specialties.forEach(specialty => {
  app.get(`/${specialty}`, (req, res) => {
    ...
    const userList = filteredUsers.map(user => `<li>${user.name}, ${user.age} años</li>`).join('');
    res.send(`<h1>${specialty.toUpperCase()}</h1><ul>${userList}</ul><a href="/">home</a>`);
  });
});
